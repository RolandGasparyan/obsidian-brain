...
class Config:
    ...
    SYMBOLS = ["BTC/USDT:USDT", "ETH/USDT:USDT", "SOL/USDT:USDT", "XRP/USDT:USDT", "AVAX/USDT:USDT"]
    TESTNET = False
...
class OneDollarSniper:
    ...
    def _find_trade(self, md):
        ...
        try:
            ...
            self.ex.open_short(md.symbol, Config.POSITION_SIZE_USD)
            
            self.trades[md.symbol] = {
                'entry': md.price,
                'time': time.time(),
                'tp_price': md.price * (1 - self.required_profit_pct / 100),
                'sl_price': md.price * (1 + self.required_loss_pct / 100),
                'votes': vote_details
            }
            self.count += 1
            ...
            print(f"   SL: ${t['sl_price']:,.4f} ({self.required_loss_pct:.4f}%)")
